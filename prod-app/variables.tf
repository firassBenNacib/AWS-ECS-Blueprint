...
variable "enable_rds_iam_auth" {
  description = "Enable IAM database authentication for the production RDS instance."
  type        = bool
  default     = true
}

variable "rds_backup_retention_period" {
  description = "RDS backup retention period in days for the production database."
  type        = number
  default     = 14
}
...
variable "cloudtrail_data_event_resources" {
  description = "CloudTrail data-event resource ARNs for the production root."
  type        = list(string)
  default     = []
}
